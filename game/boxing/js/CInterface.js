   ...
   // var _oAudioToggle;
    ...
    this._init = function(){                
       _pStartPosGuard = {x: (CANVAS_WIDTH/2), y: CANVAS_HEIGHT -70};
        ...
        _pStartPosHookR = {x: (_pStartPosGuard.x + 200), y: CANVAS_HEIGHT -70};
        var oSprite = s_oSpriteLibrary.getSprite('btn_hit_1');
        _oButHookR= new CGfxButton((_pStartPosHookR.x),_pStartPosHookR.y,oSprite,s_oStage);
        _oButHookR.setVisible(true);
        _oButHookR.addEventListener(ON_MOUSE_DOWN, this._onHookRPressed, this);
        
        _pStartPosHookL = {x: (_pStartPosGuard.x - 200), y: CANVAS_HEIGHT -70};
        var oSprite = s_oSpriteLibrary.getSprite('btn_hit_2');
        _oButHookL= new CGfxButton((_pStartPosHookL.x),_pStartPosHookL.y,oSprite,s_oStage);
        _oButHookL.setVisible(true);
        _oButHookL.addEventListener(ON_MOUSE_DOWN, this._onHookLPressed, this);
        
        _pStartPosUppercut = {x: (_pStartPosHookR.x), y: _pStartPosHookR.y - 150};
        var oSprite = s_oSpriteLibrary.getSprite('btn_hit_3');
        _oButUppercut= new CGfxButton((_pStartPosUppercut.x),_pStartPosUppercut.y,oSprite,s_oStage);
        _oButUppercut.setVisible(true);
        _oButUppercut.addEventListener(ON_MOUSE_DOWN, this._onUppercutPressed, this);
        
        _pStartPosJab = {x: (_pStartPosHookL.x), y: _pStartPosHookL.y - 150};
        var oSprite = s_oSpriteLibrary.getSprite('btn_hit_4');
        _oButJab= new CGfxButton((_pStartPosJab.x),_pStartPosJab.y,oSprite,s_oStage);
        _oButJab.setVisible(true);
        _oButJab.addEventListener(ON_MOUSE_DOWN, this._onJabPressed, this);
    ...
    this.unload = function(){
     ...
     this.refreshButtonPos = function(iNewX,iNewY){
        ...
        _oButHookR.setPosition(_pStartPosHookR.x,_pStartPosHookR.y - iNewY);
        _oButHookL.setPosition(_pStartPosHookL.x,_pStartPosHookL.y - iNewY);
        _oButUppercut.setPosition(_pStartPosUppercut.x,_pStartPosUppercut.y - iNewY);
        _oButJab.setPosition(_pStartPosJab.x,_pStartPosJab.y - iNewY);
    ...
    this._onHookRPressed = function(){
        if(!s_oGame.getPIsPunching()&& !s_oGame.getKoP()){
            s_oGame.changePlayerState(HOOK_R);
        }
    };
    
    this._onHookLPressed = function(){
        if(!s_oGame.getPIsPunching()&& !s_oGame.getKoP()){
            s_oGame.changePlayerState(HOOK_L);
        }
    };
    
    this._onJabPressed = function(){
        if(!s_oGame.getPIsPunching()&& !s_oGame.getKoP()){
            s_oGame.changePlayerState(JAB);
        }
    };
    
    this._onUppercutPressed = function(){
        if(!s_oGame.getPIsPunching()&& !s_oGame.getKoP()){
            s_oGame.changePlayerState(UPPERCUT);
        }
    };
    ...
    this.setGuardButVisibilty = function(bVisibilityValue){
        ...
        _oButHookR.setVisible(bVisibilityValue);
        _oButHookL.setVisible(bVisibilityValue);
        _oButUppercut.setVisible(bVisibilityValue);
        _oButJab.setVisible(bVisibilityValue);
    ...
    this.setPlayerHp = function(iPHp){
        _oMaskPHp.scaleY=1-iPHp/PLAYER_HP;
	s_oStageInterface.update(); 
    };
    
    this.setPlayerStamina = function(iPSt){
        _oMaskPSt.scaleY=1 -iPSt/PLAYER_STAMINA;
	s_oStageInterface.update(); 
    };
    
    this.setEnemyHp = function(iEHp){
       _oMaskEHp.scaleY=1-iEHp/ENEMY_HP;
        s_oStageInterface.update(); 
    };
